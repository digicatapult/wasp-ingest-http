...
function check_versions_consistent () {
  ...
  local HELM_VALUES_TAG_VERSION=$(yq eval '.image.tag' ./helm/wasp-ingest-http/values.yaml)
  local HELM_CHART_VERSION=$(yq eval '.version' ./helm/wasp-ingest-http/Chart.yaml)
  local HELM_CHART_APP_VERSION=$(yq eval '.appVersion' ./helm/wasp-ingest-http/Chart.yaml)
